...
public class PatchRecordVisitor extends JsonObjectFormatVisitor.Base implements SchemaBuilder {
	...
	public PatchRecordVisitor(SerializerProvider p, JavaType type, DefinedSchemas schemas) {
		super(p);
		_type = type;
		_schemas = schemas;
		...
		if (ann != null) {
			...
			try {
				for (NamedType subType : subTypes) {
					...
					PatchAvroSchemaGenerator visitor = new PatchAvroSchemaGenerator(_schemas, getProvider());
	...
	public void property(String name, JsonFormatVisitable handler,
		...
		PatchAvroSchemaGenerator wrapper = new PatchAvroSchemaGenerator(_schemas, getProvider());
	...
	protected Schema.Field schemaFieldForWriter(BeanProperty prop, boolean optional) throws JsonMappingException {
		...
		if (schemaOverride != null) {
			...
			if (fixedSize != null) {
				...
				PatchAvroSchemaGenerator visitor = new PatchAvroSchemaGenerator(_schemas, prov);
